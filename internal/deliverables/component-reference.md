...
## Review diagrams (sitemap, IA) — authoring + rendering (2026-06-09)
Review-group diagrams (e.g. the site map) are authored as standalone HTML (not part of the `/final` build) and shipped as PNGs. Two views exist: the **simple/clean** one is `worker/public/site-map/index.html`, which is also **published at `/site-map`** (utilitarian styling on purpose, so it reads as a reference, not a real site page); and the **detailed** one is `worker/public/sitemap-diagram.html` (one-line descriptions, used as the email attachment, not linked). Render PNGs with `tools/render-diagram.sh <input.html> <output.png> [width] [height]` (headless Chrome at 2x). Default the window TALL; under-sizing clips the bottom and forces a re-render.
