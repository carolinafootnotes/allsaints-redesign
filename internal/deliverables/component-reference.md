...
Review-group diagrams (e.g. the site map) are authored as standalone HTML in `worker/public/*.html` (not part of the `/final` build) and shipped as PNGs. Two views exist: `sitemap-diagram-clean.html` (names-only, with a metrics strip) and `sitemap-diagram.html` (detailed, one-line descriptions). Render with `tools/render-diagram.sh <input.html> <output.png> [width] [height]` (headless Chrome at 2x). Default the window TALL; under-sizing clips the bottom and forces a re-render.
